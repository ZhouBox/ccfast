...
public:
    ...
    typedef std::function<typename type_value::type_const_ref_value(const std::string&,
                                                                    const std::string&, char)> do_parse;
    typedef std::function<void(const std::string&, type_value&, char)> do_fast_parse;
    template <typename N>
    ...
    ParameterAnalytic() {}
    ParameterAnalytic(const StringList& paras_):m_ParaVals(paras_), m_splitFlag('&')
    {
...
void ParameterAnalytic<T>::analytic()
{
    ...
    {
        (*_ite).second = m_doParseFunctional(m_paravalsBuffer, (*_ite).first, m_splitFlag);
    }
}
...
void ParameterAnalytic<T>::fast_analytic()
{
    try
    {
        m_doParseFastFunctional(m_paravalsBuffer, m_ParaVals, m_splitFlag);
